...
class TestFormatClarificationMessage:
    """Tests for _format_clarification_message options handling (batched questions)."""
    ...
    def test_multiple_questions_are_numbered_and_joined(self, middleware):
        """A batch of 2+ questions renders each numbered, separated by a blank line."""
        args = {
            "questions": [
                {"question": "Which env?", "clarification_type": "approach_choice"},
                {"question": "Which region?", "clarification_type": "missing_info"},
            ]
        }
        result = middleware._format_clarification_message(args)
        assert "1. Which env?" in result
        assert "2. Which region?" in result
